import { Check, Clock, Copy, Eye, GitBranch, Github, Hourglass, Repeat2 } from 'lucide-react';
...
export const SessionHeaderVariantB = ({
  ...
  const renderMetaRow = (label: string, displayValue: string, copyValue: string, ariaLabel: string) => (
    <div className="group flex items-center gap-3 rounded-2xl border border-slate-100 bg-white/70 px-3 py-2">
      <span className="w-16 shrink-0 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400">{label}</span>
      <span className="min-w-0 flex-1 truncate text-xs text-slate-700" title={copyValue}>
        {displayValue}
      </span>
      <CopyButton
        onCopy={() => handleCopyMeta(copyValue)}
